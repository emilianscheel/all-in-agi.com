...
export async function POST({ request }) {
	try {
		...
		return new Response(new Blob([bytes as Uint8Array<ArrayBuffer>]), { headers: { 'content-type': 'application/pdf', 'content-disposition': 'attachment; filename="werksprung-hackathon-plan.pdf"', 'cache-control': 'no-store' } });
