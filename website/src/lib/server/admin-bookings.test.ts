...
const booking = {
	...
	customLunch: '', toolProvision: 'existing', codingTools: ['codex'], customCodingTool: '', deviceProvision: 'needed', deviceCount: 3,
	...
	basePrice: 4000, venueSurcharge: 0, lunchAdjustment: 0, toolsAdjustment: 0, devicesAdjustment: 450, totalPrice: 4450,
...
describe('admin booking CSV', () => {
	test('emits Excel-friendly UTF-8 CSV and neutralizes customer formulas', () => {
		...
		expect(csv).toContain('"Geräte-Bereitstellung","Geräteanzahl"');
		expect(csv).toContain('"needed","3"');
