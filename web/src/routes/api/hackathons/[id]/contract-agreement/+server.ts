...
export async function POST({ params, request, locals, fetch }: { params: { id: string }; request: Request; locals: App.Locals; fetch: typeof globalThis.fetch }) {
	...
	if (!current) return json({ message: 'Hackathon nicht gefunden.' }, { status: 404 });
	const readinessErrors = validateBillingDetails(current.billing);
	if (readinessErrors.length) return json({ message: 'Vor der Vertragszustimmung müssen die Rechnungsdaten vollständig hinterlegt werden.', errors: readinessErrors }, { status: 409 });
