...
import {
    defaultOfferConfiguration,
    grossTotal,
    isOfferConfiguration,
    normalizeOfferConfiguration,
...
describe("offer configuration", () => {
    test("prefills the Hitachi recipient and all requested services", () => {
        ...
        expect(selectedOfferServices(config)).toContainEqual({
            id: "preparation",
            label: "4 Tage Vorbereitungstage Konzeption",
        });
    });
    ...
    test("normalizes legacy offer links to the default client logo", () => {
        ...
        expect(normalizeOfferConfiguration(legacyConfig)).toEqual({
            ...legacyConfig,
            clientLogo: "hitachi",
        });
    });
